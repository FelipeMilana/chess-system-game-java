...
public class Knight extends ChessPiece {
	...
	private boolean canMove(Position position) {  //se o cavalo pode mover para uma determinada posição
	...
	public boolean[][] possibleMoves() {
		...
		p.setValues(position.getRow() - 1, position.getColumn() - 2);  //pega a linha e a coluna da peça rei
		...
		p.setValues(position.getRow() - 2, position.getColumn() - 1);  //pega a linha e a coluna da peça rei
		if(getBoard().positionExists(p) && canMove(p)) {  //pode mover para essa posição
			mat[p.getRow()][p.getColumn()] = true;
		}
